...
export class Migration20230831041254 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "auth_users" ("uuid" text not null default uuid_generate_v4(), "created_at" date not null, "updated_at" date not null, "version" int not null default 1, "email" varchar not null, "first_name" varchar not null, "last_name" varchar not null, "username" varchar not null, constraint "auth_users_pkey" primary key ("uuid"), constraint auth_users_username_check check (username ~ \'^[A-Za-z0-9_-]+$\'));');
    this.addSql('comment on column "auth_users"."uuid" is \'UUID v4 unique to the row.\';');
